# paṭivaṭṭati

``` toml
dict_label = "PTS"
word = "paṭivaṭṭati"
summary = "Paṭivaṭṭati & ˚vattati (intrs.) to roll or move ba"
see_also = ["paṭivattiya", "paṭivatta"]
```

Paṭivaṭṭati & ˚vattati

(intrs.) to roll or move back, to turn away from AN.iv.47 = Mil.297 (paṭilīyati paṭikutati p.); Caus. *paṭivaṭṭeti* in same meaning trs (but cp. Childers s. v. “to knock, strike”) SN.ii.265 (T spells pati˚, as also at Mil.297)  
grd *[paṭivattiya](paṭivattiya.md)* only in neg. ap˚ (q.v.)  
pp *[paṭivatta](paṭivatta.md)* (q.v.).

paṭi \+ *vr̥t*

