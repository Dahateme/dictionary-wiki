# kāyura

``` toml
dict_label = "NCPED"
word = "kāyura"
summary = "bracelet (worn on the upper arm); an arm-guard; (a"
grammar_comment = "neuter a"
see_also = ["kāyūra"]
```

bracelet (worn on the upper arm); an arm\-guard; (according to commentaries) a neck\-ornament, a necklace

--------------------

``` toml
dict_label = "PTS"
word = "kāyura"
summary = "Kāyūra & Kāyura 1. an ornamental bracket or ring w"
see_also = ["keyūra"]
```

Kāyūra & Kāyura
1. an ornamental bracket or ring worn on the upper arm (bāh’âlankāra Pv; bhuj˚ Vv) or neck (gīvāya pilandhana Ja.iii.437); a bracelet or necklace Vin.ii.106; Ja.iii.437; Ja.iv.92; Pv.iii.9#3; Vv.36#2.
2. adj as sakāyura raṭṭha having the insignia “regis” Ja.v.289 = Ja.v.486.

see also *[keyūra](keyūra.md)*, which is the only form in Sk.

